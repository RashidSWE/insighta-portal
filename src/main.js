...
async function fetchSecureData(isInitialCheck = false) {
  ...
  try {
      ...
      if (response.ok) {
          const data = await response.json();
          document.getElementById("status-message").innerText = "✅ Authenticated successfully";
          document.getElementById("secure-data").innerText = JSON.stringify(data["username"], null, 2);
      } else {
